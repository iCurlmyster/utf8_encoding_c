...
bool valid_utf8(const char *str, size_t len) {
    ...
    for (int i = 0; i < len; ) {
        ...
        int bc = handle_bytes(str, i, len, cur_t);
        if (bc == -1) {
            return false;
        }
        i += bc;
...
bool check_char_bounds_and_valid_seq(char arr[4], utf8_byte_t t) {
    ...
    switch (t) {
        ...
        case TWO:
            ...
            a = arr[0] & FIVE_FREE_BITS,
            b = arr[1] & SIX_FREE_BITS;
            return ((a << 6) | b) <= DOUBLE_MAX;
        case THREE:
            if (!NEXT_BYTE_FLAG(arr[1])) return false;
            if (!NEXT_BYTE_FLAG(arr[2])) return false;
            a = arr[0] & FOUR_FREE_BITS,
            b = arr[1] & SIX_FREE_BITS,
            c = arr[2] & SIX_FREE_BITS;
            return ((a << 12) | (b << 6) | c) <= TRIPLE_MAX;
        case FOUR:
            if (!NEXT_BYTE_FLAG(arr[1])) return false;
            if (!NEXT_BYTE_FLAG(arr[2])) return false;
            if (!NEXT_BYTE_FLAG(arr[3])) return false;
            a = arr[0] & THREE_FREE_BITS,
            b = arr[1] & SIX_FREE_BITS,
            c = arr[2] & SIX_FREE_BITS,
            d = arr[3] & SIX_FREE_BITS;
            return ((a << 18) | (b << 12) | (c << 6) | d) <= QUAD_MAX;
        default:
            return false;
    }
}
